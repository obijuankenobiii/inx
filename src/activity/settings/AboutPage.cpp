...
void AboutPage::renderWithRefresh() {
  ...
  if (onCheckForUpdates) {
    ...
    renderer.rectangle.fill(btnX, yPos, btnW, btnH, false);
    renderer.rectangle.render(btnX, yPos, btnW, btnH, true);
    const char* updateLabel = "Update";
    const int tw = renderer.text.getWidth(ATKINSON_HYPERLEGIBLE_12_FONT_ID, updateLabel, EpdFontFamily::BOLD);
    const int tx = btnX + (btnW - tw) / 2;
    const int ty = yPos + (btnH - renderer.text.getLineHeight(ATKINSON_HYPERLEGIBLE_12_FONT_ID)) / 2;
    renderer.text.render(ATKINSON_HYPERLEGIBLE_12_FONT_ID, tx, ty, updateLabel, true, EpdFontFamily::BOLD);
  }
