 ...
 */
BookmarkActivity::BookmarkActivity(GfxRenderer& renderer, MappedInputManager& mappedInput,
                                   const std::vector<Bookmark>& bookmarks, const std::string& bookTitle,
                                   int currentSpine, int currentPage, SelectCallback onSelect, DeleteCallback onDelete,
                                   BackCallback onBack)
 ...
 */
void BookmarkActivity::onExit() { renderer.clearScreen(); }
...
void BookmarkActivity::renderScreen() {
  ...
  for (int i = startIndex; i < endIndex; i++) {
    ...
    renderer.drawText(ATKINSON_HYPERLEGIBLE_10_FONT_ID, screenWidth - pageWidth - 50, textY + 5, pageInfo,
                      isSelected ? 0 : 1);
