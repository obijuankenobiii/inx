  ...
  if (!coverDrawn) {
    if (!title.empty()) {
      ...
      while (!remaining.empty() && lineCount < 3) {
        ...
        size_t spacePos = 0;
        while (spacePos != std::string::npos && lineCount < 3) {
          ...
          if (lineWidth + wordWidth <= maxWidth) {
            if (!line.empty()) line += " ";
            line += word;
            lineWidth += wordWidth + (line.empty() ? 0 : renderer.getTextWidth(ATKINSON_HYPERLEGIBLE_12_FONT_ID, " "));
...
void StatisticActivity::render() const {
  ...
  if (allBooksStats.empty()) {
    ...
    int visibleRows = 2;
    int availableHeight = screenHeight - startY - (GRID_SPACING * 2);
    ...
    for (int row = 0; row < 2; row++) {
      ...
      int coverItemY = startY + GRID_SPACING + row * (containerHeight + GRID_SPACING) + 35;

      int detailsCol = 1;
      int detailsItemX = GRID_SPACING + detailsCol * (containerWidth + GRID_SPACING);
      int detailsItemY = coverItemY - 50;
      ...
      if (progress >= 0) {
        ...
        renderer.fillRect(coverWidth - 20, barY - 20, renderer.getTextWidth(ATKINSON_HYPERLEGIBLE_8_FONT_ID, pText) + 8, renderer.getTextWidth(ATKINSON_HYPERLEGIBLE_8_FONT_ID, pText) + 8, false, true);
        renderer.drawText(ATKINSON_HYPERLEGIBLE_8_FONT_ID,coverWidth - 13, barY - (renderer.getTextWidth(ATKINSON_HYPERLEGIBLE_8_FONT_ID, pText) / 2), pText);
      ...
      snprintf(buffer, sizeof(buffer), "%d", allBooksStats[bookIndex].totalPagesRead);
      ...
      snprintf(buffer, sizeof(buffer), "%d", allBooksStats[bookIndex].totalChaptersRead);
      renderer.drawText(VALUE_FONT, textX, currentY, buffer, true, EpdFontFamily::BOLD);
      renderer.drawText(LABEL_FONT, textX, currentY + 45, "Chapters", true);
      currentY += 80;

      if (allBooksStats[bookIndex].avgPageTimeMs > 0) {
        snprintf(buffer, sizeof(buffer), "%ds", allBooksStats[bookIndex].avgPageTimeMs / 1000);
      } else {
