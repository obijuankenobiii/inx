...
void FullScreenMessageActivity::onEnter() {
  ...
  const auto height = renderer.text.getLineHeight(ATKINSON_HYPERLEGIBLE_10_FONT_ID);
  const auto top = (renderer.getScreenHeight() - height) / 2;

  renderer.clearScreen();
  renderer.text.centered(ATKINSON_HYPERLEGIBLE_10_FONT_ID, top, text.c_str(), true, style);
