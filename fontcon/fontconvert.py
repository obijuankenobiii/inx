...
for i_start, i_end in intervals:
    for code_point in range(i_start, i_end + 1):
        ...
        glyph = GlyphProps(
            width = bitmap.width,
            height = bitmap.rows,
            # Legacy firmware headers store whole-pixel advanceX in uint8_t.
            # Newer font packs use 12.4 fixed-point from linearHoriAdvance.
            advance_x = norm_ceil(face.glyph.advance.x) if args.legacy_header else fp4_from_ft16_16(face.glyph.linearHoriAdvance),
...
if kern_map and not args.legacy_header:
...
if ligature_pairs and not args.legacy_header:
