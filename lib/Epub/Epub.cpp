 ...
 * copies JPEG covers to `thumb.jpg`. Resized JPEG thumbnails are generated by the web importer when available.
 */
bool Epub::generateThumbBmp() const {
  ...
  if (!isJpegFile(coverHref)) {
    ...
    sourceFile.close();
    SdMan.remove(tempPath.c_str());
    success = extractItemToPath(coverHref, getThumbJpegPath(), 4096);
    Serial.printf("[EBP] Thumbnail JPEG %s: %s\n", success ? "copied" : "failed", getThumbJpegPath().c_str());
    return success;
  }
